...
- Search the current directory for agent pipeline files: `crew.py`, `agents.yaml`, `tasks.yaml`, `*.json` workflows, or any file containing `Agent(`, `@agent`, `crewai`, `n8n`
...
Only include CRITICAL, HIGH, and MEDIUM. Skip LOW and N/A entirely.
...
A numbered list of the top 3 things to do right now, one line each.

---

Keep it tight. Lead with the fix. Skip attack theory — the user just needs to know what to paste and where.
